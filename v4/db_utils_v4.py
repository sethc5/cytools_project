#!/usr/bin/env python3
"""db_utils_v4.py — SQLite database layer for pipeline v4.

Fresh database (cy_landscape_v4.db) with expanded schema for
...
DEFAULT_DB = os.path.join(os.path.dirname(__file__), "cy_landscape_v4.db")
...
class LandscapeDB:
    """SQLite wrapper for the v4 CY landscape database."""
    ...
    def __repr__(self):
        total = self._conn.execute(
            "SELECT COUNT(*) FROM polytopes").fetchone()[0]
        return f"<LandscapeDB v4: {self.db_path} ({total} polytopes)>"
...
if __name__ == "__main__":
    ...
    if not os.path.exists(db_path):
        print(f"Creating new v4 database: {db_path}")
