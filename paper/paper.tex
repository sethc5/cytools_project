...
\author{%
  [Author A]$^{a}$\quad [Author B]$^{b}$
  \\[6pt]
  \small $^a$[Institution A]\\
  \small $^b$[Institution B]
  \\[4pt]
...
A deep bundle analysis of h26/P11670 via LP-based slope-stability
and the D3-tadpole condition reveals a structural obstruction:
all perturbative SU(4) monad bundles with integer charges generate
$\ch_2(V)_k \gg c_2(TX)_k$ due to the large intersection numbers
$\max|\kappa_{kab}|/2 = 43.5$, an exact result independent
of sampling depth.
...
\begin{equation}
  n_{\rm gen} = \frac{|\chi(X)|}{2}
    = \frac{|h^{1,1}(X) - h^{2,1}(X)|}{1}\,.
  \label{eq:ngen}
\end{equation}
(The factor-of-2 difference between $\chi = 2(h^{1,1}-h^{2,1})$
and $n_{\rm gen}$ accounts for the fact that a CY$_3$ with
$h^{1,1} > h^{2,1}$ yields $h^{1,1}-h^{2,1}$ net generations
of the $\mathbf{27}$ of $E_6$; after the Wilson-line breaking
$E_6 \to \mathrm{SU}(3)\times\mathrm{SU}(2)\times\mathrm{U}(1)$,
each $\mathbf{27}$ decomposes into one SM generation plus
vector-like exotics, and the net count is exact.)
...
\label{prop:boundary}
  No polytope with $h^{1,1} \in \{29,30,31,32\}$ achieves
  SM composite score $\geq 70$ in the $\chi=-6$ KS database.
  The maximum score in this band is $87$ (at $h^{1,1}=29$),
  but all such entries have $h^{1,1}_{\rm eff} > 22$ and fail
  the T0 geometric gate, achieving effective score $\leq 10$
  at T2.
\end{proposition}
...
file for $h^{1,1}=26$.
Its Hodge numbers are $h^{1,1}=26$, $h^{2,1}=26$ (so $\chi=-6$
...
200 independent FRSTs are computed.
$c_2$-stable fraction: $0.0333$ (2/60 triangulations share the modal
$c_2$ hash in the 60-sample stability recompute).
$\kappa$-stable fraction: $0.0333$.
...
$h^{1,1}$ & poly\_idx & $h^{2,1}$ & $h_{\rm eff}$ & $n_{\rm clean}$
           & has\_SM & has\_GUT \\
\midrule
29 & 8423  & 26 & 24 & 30 & \checkmark & \checkmark \\
24 & 868   & 21 & 22 & 24 & \checkmark & \checkmark \\
27 & 9192  & 24 & 24 & 22 & \checkmark & \checkmark \\
27 & 4102  & 24 & 24 & 22 & --         & --         \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{equation}
  P(\text{same hash}) \;=\; \sum_\tau \left(\frac{N_\tau}{N_{\rm FRST}}\right)^2
    \;\leq\; \frac{1}{N_{\rm FRST}},
  \label{eq:hash_prob}
\end{equation}
where $N_\tau$ is the number of FRSTs with hash $\tau$.
For $h^{1,1}=25$--28 entries with $N_{\rm FRST} \sim 10^{8}$--$10^{10}$,
this gives expected $c_2$-stable $\approx n_{\rm T3}/N_{\rm FRST} \ll 1$,
consistent with the 87.5\% of T3 entries having $c_2$-stable $< 0.10$.
...
\centering
\caption{%
  SU(4) monad LP scan on the five highest-priority T4-verified
  $\chi=-6$ polytopes.  $\kappa_{\rm max}=\max_{k,a,b}|\kappa_{kab}|$;
...
\midrule
\textbf{All five} & --- & --- & --- & 1,3 & $\sim$22\,M
  & \textbf{82\,630} & \textbf{0} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Lower-score polytopes}: Theorem~\ref{thm:no_go}
    (Section~\ref{sec:monad_universal}) extends the obstruction to all
    four other priority T4-verified entries ($\hone_{\rm eff}=19$--20),
    with excess factors $1.5$--$2.9\times$ even at minimal charges.
    The obstruction is therefore generic across the high-score tail of
    the $\chi=-6$ landscape, not specific to h26/P11670.
    Polytopes at significantly lower $h^{1,1}\leq 16$ have smaller
    intersection numbers and could in principle satisfy
    $\ch_2(V)\leq c_2(TX)$, at the cost of a substantially lower
    composite score (cf.\ h16/P118 at score~76).
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Monad bundles at higher $k_{\rm max}$}: The scan
    at $k_{\rm max}=2$ finds 612 slope-feasible candidates, all
    tadpole-obstructed.  Raising to $k_{\rm max}=3$ increases the
    number of slope-feasible candidates but also enlarges $\ch_2(V)$,
    making tadpole satisfaction harder.  A rigorous $k_{\rm max}=3$
    scan is needed.
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{D3-tadpole obstruction}: perturbative SU(4) monads
    on h26/P11670 are exactly obstructed; the F-theory lift via the
    su(10) F11 fibration is the most promising path to a full vacuum.
\end{enumerate}
