#!/usr/bin/env python3
"""
pipeline_h14_P2.py — Full deep-analysis pipeline for our #1 candidate.

Target: h11=14, polytope index 2  (h21=17, χ=-6)
  - 268 clean h⁰=3 bundles (most of any polytope)
  - 828 bundles with h⁰≥3
  - 14,608 χ=3 bundles scanned
  - 3 K3 fibrations, 1 elliptic fibration
  - 176 distinct D³ values across clean bundles
  - T2 score = 41
...
def run_stage1():
    ...
    polys = list(cy.fetch_polytopes(h11=TARGET_H11, h21=TARGET_H21,
                                    lattice='N', limit=10))
    print(f"  h11={TARGET_H11}, h21={TARGET_H21} polytopes: {len(polys)}")
...
def run_stage2(cyobj, pts, ray_indices, polytope):
    ...
    h11 = cyobj.h11()
    n_toric = pts.shape[0]
    h11_eff = None  # set after div_basis is computed
    ...
    for a_idx, a in enumerate(div_basis):
        ...
        c2D = c2[a_idx]
        chi_D = D3 / 6.0 + c2D / 12.0  # not the CY chi, but χ(O_D)
    ...
    if swiss_candidates:
        swiss_candidates.sort(key=lambda x: x[3])  # sort by ratio
...
def run_stage3(cyobj, pts, ray_indices, div_basis, intnums, c2, mori,
    ...
    # ── Compute h⁰ via Koszul for all χ=+3 bundles ──
    # ── Compute h³ = h⁰(-D) via Serre for χ=-3 bundles ──
    ...
    print(f"\n  Top 25 bundles by h⁰:")
    print(f"  {'D (basis coords)':<55} {'χ':>4} {'h⁰':>4} {'h³':>4} {'D³':>8}  {'status'}")
    print(f"  {'─' * 85}")
    for r in top:
        nz = [(i, int(r['D_basis'][i])) for i in range(h11_eff) if r['D_basis'][i] != 0]
...
def run_stage4(results, h11_eff):
    ...
    # ── Breakdown of h⁰≥3 bundles by (h⁰, h³, h¹-h²) ──
    print(f"\n  Cohomology breakdown of all h⁰≥3 bundles:")
    print(f"    {'h⁰':>4} {'h³':>4} {'h¹-h²':>6} {'count':>6}  {'interpretation'}")
    print(f"    {'─' * 55}")
    ...
    # ── Show a few representative clean bundles with physical interpretation ──
...
def print_scorecard(cyobj, results, bundles, d3_unique,
    ...
    score("D³ diversity (clean)", len(d3_unique), lambda x: x >= 10, 1,
          f"range [{d3_unique[0]:.0f}, {d3_unique[-1]:.0f}]" if d3_unique else "")
  ...
  derived from these line bundles:
    ...
    {"• Swiss cheese structure (τ=" + f"{best_tau:.1f}" + ") enables LVS" if has_swiss else ""}
      moduli stabilization → hierarchy m_soft ~ M_P/V ~ TeV
...
def main():
    ...
    print(f"  {CYAN}Ranked #1 of 177 T2-screened polytopes{RESET}")
    print(f"  {CYAN}268 clean h⁰=3 bundles — most of any CY in the χ=-6 landscape{RESET}")
