#!/bin/bash
# batch_t3_sweep.sh — T3 deep sweep on score=70-79 T2-only candidates
# Targets all 928 T2-only entries at score>=70 (no T3 re-runs in seed).
# Uses CYTools CGI API (no --local-ks needed).
#
# Usage: bash v6/batch_t3_sweep.sh
# Env:   WORKERS=14 TOP=300 bash v6/batch_t3_sweep.sh
# Output: v6/t3_sweep_results.db (new T3 rows for merge)

set -e
cd "$(dirname "$0")"

DB=cy_landscape_t3_sweep.db
SEED=t3_sweep_seed.db
LOG=results/batch_t3_sweep.log
TOP=${TOP:-300}           # score 75-79 (241 entries) + top 59 of score 74 ≈ 8 hrs
...
echo "=== T3 SWEEP START: $(date -u) ===" | tee "$LOG"
...
t3  = con.execute("SELECT COUNT(*) FROM polytopes WHERE tier_reached='T3'").fetchone()[0]
t2  = con.execute("SELECT COUNT(*) FROM polytopes WHERE tier_reached='T2'").fetchone()[0]
print(f"Post-run: T3={t3}  T2-remaining={t2}  new-T3={t3}")
rows = con.execute("""
...
echo "" | tee -a "$LOG"
echo "Exporting t3_sweep_results.db for local merge..." | tee -a "$LOG"
python3 - "$DB" <<'PYEOF'
import sqlite3, sys, os

src = sqlite3.connect(sys.argv[1])
if os.path.exists('t3_sweep_results.db'):
    os.remove('t3_sweep_results.db')
dst = sqlite3.connect('t3_sweep_results.db')
...
# Export all T3 rows (newly classified during sweep)
...
dst.commit()
sz = os.path.getsize('t3_sweep_results.db')
print(f"t3_sweep_results.db: {len(rows)} T3 rows, {len(fibs)} fib rows, {sz//1024} KB")
PYEOF
...
echo ""
echo "Next steps:"
echo "  scp root@95.216.246.55:\$(docker inspect -f '{{.GraphDriver.Data.MergedDir}}' funny_davinci)/workspaces/cytools_project/v6/t3_sweep_results.db v6/"
echo "  OR: docker cp funny_davinci:/workspaces/cytools_project/v6/t3_sweep_results.db /tmp/ && scp root@95.216.246.55:/tmp/t3_sweep_results.db v6/"
echo "  python3 v6/merge_t3_results.py --results v6/t3_sweep_results.db"
