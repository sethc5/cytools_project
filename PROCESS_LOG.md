...
## 2026-03-07 — B-41 Paper Expansion (Session 2): 21pp → 29pp

**Goal**: Expand paper toward 35-50pp JHEP target. Continue from 21pp clean build.

**Steps executed**:

1. **§3.4 Koszul formula + Yukawa** (~35 lines added):
   - Explicit Koszul SES (eq:koszul), long exact sequence (eq:les), BDS theorem (eq:bds)
   - Census complexity: $5^r$ → pruned to $10^4$–$10^6$ per polytope
   - Yukawa tensor eqs (eq:yukawa, eq:yukawa2), SVD hierarchy definition

2. **Appendix A expanded** (~120 lines): all 9 pitfalls promoted from table rows to full
   subsubsections with code examples (A1 fetch limit, A2 FRST seed, A3 c2 blow-down,
   A4 Yukawa asymmetry, A5 score drift, A6 timeout, A7 rigid divisors, A8 id collision, A9 KS ordering)

3. **§3.3 T1 LVS physics** (~60 lines): LVS Kähler potential K (eq:lvs_K), superpotential W (eq:lvs_W),
   LVS minimum (eq:lvs_min), Swiss-cheese criterion, dP threshold derivation

4. **§7.2 Monad LP** (~30 lines): Slope stability condition (eq:slope), explicit LP formulation (eq:lp),
   scipy.linprog implementation note

5. **§3.5 Fibration detection** (new subsection, ~50 lines): Mori-cone 2D-subcone algorithm,
   fiber test criteria (F²=0, F·H>0, χ(S)), Kodaira type table, gauge algebra identification

6. **§7.1 Direct-sum** expanded (~25 lines): Hoppe criterion formal statement, SU(4)/SU(5)
   result with interpretation (large Mori cone kills dual cone)

7. **Appendix D** (new, ~80 lines): SM score component derivations for all 10 components;
   score formula version history and stability note

8. **§2.4 chi=-6 motivation** (new subsection, ~40 lines): three-generation formula (eq:ngen),
   mirror symmetry sign convention, KS distribution explanation

9. **§5.6 Fibration cluster** (new subsection, ~40 lines): gauge algebra stats table for top-37,
   h26/P11670 I10 fiber highlight, so(10)/e7 entries, Mordell-Weil rank bound

10. **§8.4 F-theory expanded** (~50 lines): Weierstrass model (eq:weierstrass), Tate algorithm steps,
    G4 tadpole formula (eq:f_tadpole), spectral cover factorisation, base identification note

11. **§4.2 per-h11 stats table** (Table~\ref{tab:scoreband}): per-h11 mean/max score and SM/GUT
    fraction for h11=19-29 (queried from DB)

12. **§6.3 FRST counting** (new subsection): P(same hash) formula (eq:hash_prob), score
    triangulation-invariance explanation

13. **Added refs**: DonagiWijnholt2008 (spectral cover), MordellWeilHetF (Grimm-Hayashi 2012)

14. **Fixed**: `\usepackage[expansion=false]{microtype}` (font expansion error from mathfrak in tables);
    `\bC` macro added

**Result**: 21pp → 29pp, 582KB, zero errors. Build: `pdflatex × 2 + bibtex + pdflatex × 2`.
...
### Root cause
The §22 merge (DELETE+INSERT from Hetzner) reintroduced NULL `tier_reached` values from the §21 fresh-scan rows. The §21 container DB had been generated by a pipeline version that didn't always set `tier_reached` before row creation. The §22 raw INSERT preserved those NULLs. The local one-time fix from §20 (Issue A) didn't survive the replace-all merge.

### v6 math audit (prompted by h18/h19 stall)
Full import chain traced: `pipeline_v6 → cy_compute_v6 → cy_compute_v5 → archive/v2`. `compute_yukawa_texture` is **byte-for-byte identical** across all versions. No math regression. h18/P315 (`h11_eff=19>h11=18`) fails in T2 worker due to array bounds on unfavorable geometry — not recoverable; historic score=80 was a different-pipeline artifact.

### db_utils_v6 fix (tier demotion prevention)
Added `tier_reached` to `MONOTONIC_MAX` in the v6 subclass:
```python
MONOTONIC_MAX = _LandscapeDB_v5.MONOTONIC_MAX | {'tier_reached'}
```
`T0<T1<T2<T3` sorts correctly alphabetically so `MAX()` on TEXT always keeps the highest tier. Committed `cd05797`, deployed to Hetzner.

### Tier repair (local DB)
```sql
UPDATE polytopes SET tier_reached='T2' WHERE tier_reached IS NULL AND n_clean IS NOT NULL;
-- 20,267 rows promoted to T2

UPDATE polytopes SET tier_reached='T0' WHERE tier_reached IS NULL AND n_clean IS NULL AND yukawa_rank IS NOT NULL;
-- 39,622 rows promoted to T0

-- 704,875 NULL rows remain: truly unprocessed placeholder rows with no data
```

**Result**: T2: 14,164→**34,431** | T0: 2,285,255→**2,324,877** | champion tier_reached restored to T2.
Hetzner container already clean (0 NULLs) — no repair needed there.

### Notable T1 backlog discovered
6,799 T1-pass rows with `n_clean_est>0` never had T2 run — entire h20 (2,021) and h21 (2,795) populations. Top candidates: h20/P31039 and h20/P53414 (n_clean_est=102 each), h20/P70906 (max_h0=6). These are the richest unexplored territory.

### Key insight: local DB has h20–h22 T1 data the container lacks
The local DB holds T1 results from very early scans (pre-§21) covering h20/h21/h22 in bulk. The container only has what Hetzner ran. A targeted T2 sweep of h20–h22 T1 backlog would be high-ROI.
...
## 2026-03-07 — Champion Monad LP Slope Filter + Tadpole Recheck (B-45)

**Context**: After 9M random-sampling monad trials (k=2: 3M, k=3: 6M) returned 0 slope-stable SU(4) bundles, replaced random J-sampling with LP-based gradient optimization.

### LP Slope Filter (`champion_monads_lp.py`)

**Algorithm**: For each χ=±3 monad candidate (B,C), use L-BFGS-B with exp(v) parametrization to minimize the log-sum-exp slope margin F(v) = softmax(μ_B) − softmin(μ_C). Analytic gradients. 20 random starts per candidate. F < 0 ⟹ slope-feasible.

**Execution**: Hetzner1 (95.216.246.55), container `funny_davinci`, Python 3.12 + scipy 1.17.1. Configs (5,1),(6,2),(7,3), k_max=3, n_sample=2M, n_starts=20.

**Results**:
- Sampled: 5,989,706
- Slope-feasible (LP margin < 0): **612**
- Vol > 0 (valid Kähler cone): **406**
- Elapsed: 4435s (~74 min)

**Commits**: `dc0f145` (champion_monads_lp.py initial), `946931b` (fix args= bug in scipy minimize)

### Tadpole Bug Discovery and Correction (`recheck_tadpole.py`)

**Bug**: Original `check_c2_tadpole()` used linear formula `ch₂(O(β))_k = κ_{kab}J^b β^a`. Since c₁(B) = c₁(C) (SU(4) constraint), this always cancels to give ch₂(V) ≡ 0, so n_D3 = Σc₂(TX) = 66.0 **exactly** for all 612 candidates (std=0). The formula computes the slope, not ch₂.

**Fix**: Correct quadratic formula `ch₂(O(β))_k = (1/2) κ_{kab} β^a β^b`. Implemented in `recheck_tadpole.py`. Re-checked 406 vol>0 candidates.

**Corrected results**:
- Tadpole OK: **0 / 406**
- c₂(V)_max range: 167 – 1654 (vs c₂(TX)_max = 24)
- n_D3 range: [−3006, 0], mean = −702

**Physical diagnosis**: With charges |β^a| ≤ 13 (as generated by the random search), quadratic ch₂ terms grow far beyond the manifold's c₂(TX) budget. The h11=28 Picard rank means the κ_{kab} tensor (203 entries) amplifies charge products by large factors. This is a genuine physical obstruction: SU(4) monads with O(1) integer charges cannot satisfy both slope-stability AND D3-tadpole on h26/P11670.

**Next**: B-46 — Restrict charges to |β^a| ≤ 1 and build tadpole as hard LP constraint.

---

## 2026-03-07 — §31 Top-37 Priority Monad LP Scan (B-46)

**Goal**: Extend the LP monad slope-feasibility + D3-tadpole scan from h26/P11670 (B-45) to all 4 priority T4 entries with h11_eff ≤ 20: h22/P682 (h11_eff=19), h23/P36 (h11_eff=19), h21/P9085 (h11_eff=19), h25/P860 (h11_eff=20). These were flagged as having smaller κ tensors than the champion and thus potentially escaping the D3 obstruction.

**Script**: `v6/monad_scan_top37.py` (745 lines, new). Generalizes `champion_monads_lp.py`:
- Falls back to `ct.fetch_polytopes` (internet) when local KS index absent
- Structural κ_max vs c2_max pre-check per component
- Phase 1: 500K random χ=±3 monad trials (c1=0 enforced)
- Phase 2: L-BFGS-B slope-margin optimization (20 starts, v bounded ±3)
- Per-entry JSON checkpoints + global summary
- DB columns added: `monad_lp_k_max`, `monad_lp_n_sample`, `monad_lp_slope_feasible`, `monad_lp_tadpole_ok`, `monad_lp_kappa_max`, `monad_lp_c2_max`, `monad_lp_elapsed_s`, `monad_lp_timestamp`

**Execution**: Local Dell (PID 1675234), 103% CPU, 12:56–12:55 UTC. Total: 3585s (~60 min).

**Results per entry** (SU(4) monads, configs (5,1)+(6,2)+(7,3), k_max=3, n_sample=500K):

| Entry | h11_eff | κ_max | c2_max | Elapsed | Slope-feasible | Tadpole-OK |
|-------|---------|-------|--------|---------|----------------|------------|
| h22/P682  | 21 | 14 | 26 | 908s  | 221 | **0** |
| h23/P36   | 21 | 24 | 34 | 723s  | 137 | **0** |
| h21/P9085 | 21 |  9 | 46 | 739s  | 180 | **0** |
| h25/P860  | 25 |  9 | 18 | 1216s | 187 | **0** |

**Total tadpole-OK: 0 / 725 slope-feasible candidates across all 4 entries.**

**Physical diagnosis**: Slope stability (LP margin < 0) is achievable at high rates (74–79% of χ-candidates have a slope-feasible Kähler class J), but ch₂(V) per-component invariably exceeds c₂(TX). The ch₂(V)_k ~ n_B k_max² κ_{kab}/2 contribution with k_max=3, n_B=5, κ_max=9–24 generates ch₂ values of order 100–500, far exceeding c₂(TX)_k which ranges −12 to +46. This is a structural obstruction: it does not depend on geometry details, only on charge magnitudes.

**DB update**: 5 rows updated in `polytopes` (h21/P9085, h22/P682, h23/P36, h25/P860, h26/P11670).
