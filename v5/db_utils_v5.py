#!/usr/bin/env python3
"""db_utils_v5.py — SQLite database layer for pipeline v5.

Points at the same cy_landscape_v4.db by default (v5 is a scoring/analysis
upgrade, not a schema break). Adds migration for v5-specific columns:
  - tri_n_tested: number of random triangulations tested (compute_tri_stability)
  - tri_c2_stable_frac: fraction with same c₂ as default placing tri
  - tri_kappa_stable_frac: fraction with same κ nonzero pattern
...
Schema notes:
  ...
  - _migrate() adds new columns if missing (safe for existing v4 DBs)

Usage:
    from db_utils_v5 import LandscapeDB

    db = LandscapeDB()                           # opens cy_landscape_v4.db
...
# Point at the same v4 database — v5 is a scoring upgrade, not a migration.
DEFAULT_DB = os.path.join(os.path.dirname(os.path.dirname(__file__)),
                          "v4", "cy_landscape_v4.db")
...
CREATE TABLE IF NOT EXISTS polytopes (
    ...
    -- ── T3 v5: triangulation stability (compute_tri_stability) ──
    tri_n_tested    INTEGER,           -- # random tris tested for c₂/κ stability
    tri_c2_stable_frac REAL,           -- fraction with same c₂ as default tri
    tri_kappa_stable_frac REAL,        -- fraction with same κ nonzero pattern
...
class LandscapeDB:
    """SQLite wrapper for the v5 CY landscape database."""
    ...
    def __init__(self, db_path=None):
        ...
        self._conn.execute("PRAGMA journal_mode=WAL")  # reduces lock contention under concurrent writers
    ...
    def _migrate(self):
        """Add v5 columns to an existing v4 database if missing.

        Safe to call multiple times — checks column existence first.
        """
        cursor = self._conn.execute("PRAGMA table_info(polytopes)")
        ...
        for col_name, col_type in migrations:
            if col_name not in existing:
                self._conn.execute(
                    f"ALTER TABLE polytopes ADD COLUMN {col_name} {col_type}")
    ...
    def __repr__(self):
        total = self._conn.execute(
            "SELECT COUNT(*) FROM polytopes").fetchone()[0]
        return f"<LandscapeDB v5: {self.db_path} ({total} polytopes)>"
...
if __name__ == "__main__":
    ...
    if not os.path.exists(db_path):
        print(f"Creating new v5 database: {db_path}")
